  ...
  return (
    <div className="flex w-full flex-col space-y-3">
      ...
      {questionsArray.map((question) => (
        <QuestionAnswersCard question={question} key={question.questionIndex} />
      ))}
    </div>
  );
}
...
function QuestionAnswersCard({ question }) {
  ...
  const handleClick = async () => {
    ...
    const rank = rankedAnswers.map((answer) => answer.modelId);

    // const response = await axios.post(`http://localhost:3000/api/tasks/scores/updateOnceAd`, {
    //   id: "1",
    //   index: questionIndex,
    //   rank:
    // });
        ...
        {(provided) => (
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              {isRanked ? (
                <>
                  <span className="font-bold text-green-500">已排序</span>
                </>
              ) : (
                <span className="font-bold text-red-500">未排序</span>
              )}
            </div>
            ...
            <div className="flex justify-center space-x-5">
              ...
              {isRanked ? (
                <>
                  <button className="btn  text-sm">↺回复排序</button>
